...
% Compile: xelatex -shell-escape USER-GUIDE.tex
...
\definecolor{imgborder}{HTML}{F1F3F5}
...
\newcommand{\eltaginfo}[1]{\tcbox[on line,size=small,colback=black!10,colframe=black!50,boxrule=0.5pt,arc=2pt,left=3pt,right=3pt,top=0.5pt,bottom=0.5pt]{\small #1}}
\newcommand{\eltagwarning}[1]{\tcbox[on line,size=small,colback=black!10,colframe=black!50,boxrule=0.5pt,arc=2pt,left=3pt,right=3pt,top=0.5pt,bottom=0.5pt]{\small #1}}
...
\newcommand{\screenshot}[2][0.8\textwidth]{%
  \IfFileExists{../images/#2}{%
    \begin{center}%
      ...
      \fcolorbox{imgborder}{white}{\includegraphics[width=#1,keepaspectratio]{../images/#2}}%
    \end{center}%
  }{%
...
\fancyhead[R]{\small SmartTrans User Guide}
...
\RedeclareSectionCommand[
  beforeskip=-0.3ex plus -0.1ex minus -0.1ex,
...
\begin{tcolorbox}[
  colback=black!8, colframe=black!60,
  leftrule=4pt, rightrule=0pt, toprule=0pt, bottomrule=0pt,
  arc=0pt, outer arc=0pt,
  before skip=12pt, after skip=16pt,
  left=12pt, right=12pt, top=8pt, bottom=8pt,
]
  \textbf{Online Demo:}\par
  The SmartTrans system is deployed and accessible at:\par
  \medskip
  \begin{center}
    \large\url{http://1.95.119.213:28123/}
  \end{center}
  \smallskip
  \small Open this address in your browser to follow along with every hands-on step in this guide. No local installation required.
\end{tcolorbox}

\begin{tipbox}
  \textbf{Login credentials:} The system requires a logged-in account. Please refer to the \textbf{green sticky note on the laptop} for the username and password.
\end{tipbox}
...
\screenshot[0.35\textwidth]{image-03.png}
